...
This InSpec Profile was created to facilitate testing and auditing of `Microsoft SQL Server 2022 Instance`
...
The Microsoft SQL Server 2022 Instance STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Table of Contents
=================
- [Microsoft SQL Server 2022 Instance Security Technical Implementation Guide](#microsoft-sql-server-2022-instance-security-technical-implementation-guide)
...
Requirements associated with the Microsoft SQL Server 2022 Instance STIG are derived from the
...
The Microsoft SQL Server 2022 Instance STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the Microsoft SQL Server 2022 Instance STIG profile.
...
```bash
# Using `ssh` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/microsoft-sql-server-2022-instance-stig-baseline/archive/main.tar.gz --input-file=<your_inputs_file.yml> -t ssh://<hostname>:<port> --sudo --reporter=cli json:<your_results_file.json>

# Using `winrm` transport
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/microsoft-sql-server-2022-instance-stig-baseline/archive/master.tar.gz --target winrm://<hostip> --user '<admin-account>' --password=<password> --input-file=<path_to_your_inputs_file/name_of_your_inputs_file.yml> --reporter=cli json:<path_to_your_output_file/name_of_your_output_file.json>
```
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/microsoft-sql-server-2022-instance-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive microsoft-sql-server-2022-instance-stig-baseline
...
```bash
cd microsoft-sql-server-2022-instance-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive microsoft-sql-server-2022-instance-stig-baseline --overwrite
